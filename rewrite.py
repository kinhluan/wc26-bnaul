        ...
        while True:
            ...
            while True:
                try:
                    ...
                    print("\\n[ASK AGENT] EOF Error. Returning default.")
                    return adjustment
...
# Extract content before def call_llm_api
start_idx = content.find("def call_llm_api(")
if start_idx == -1:
    print("Error: Could not find call_llm_api")
    sys.exit(1)

# Extract content after the function ends
# The function ends at "return 0.0" right after "except ImportError:"
except_idx = content.find("except ImportError:", start_idx)
end_str = "        return 0.0\n"
end_idx = content.find(end_str, except_idx) + len(end_str)

new_content = content[:start_idx] + new_call_llm_api + "\n" + content[end_idx:]
