...
show_menu() {
    ...
    echo "  ${BOLD}me${NC}                    Show agent info and standings"
    echo "  ${BOLD}fixtures${NC}              List open fixtures"
    echo "  ${BOLD}predict${NC} <match_id>   Submit prediction for a match"
    echo "  ${BOLD}check${NC}                 Check all submitted predictions"
    echo "  ${BOLD}mine${NC} <match_id>      View specific prediction"
    echo "  ${BOLD}fifa-data${NC}             Fetch live FIFA data"
    echo ""
    
    echo -e "${YELLOW}━━━ ANALYSIS COMMANDS (Analyze Before Playing) ━━━${NC}"
    echo "  ${BOLD}predict-model${NC} <home> <away>  Run prediction model"
    echo "  ${BOLD}strategy-demo${NC}                  Show strategy demonstration"
    echo "  ${BOLD}backtest-demo${NC}                  Run backtest demonstration"
    echo ""
    
    echo -e "${BLUE}━━━ MONITORING COMMANDS (Set and Forget) ━━━${NC}"
    echo "  ${BOLD}monitor${NC}               Run news monitor (dry-run)"
    echo "  ${BOLD}monitor-live${NC}          Run news monitor (live resubmit)"
    echo "  ${BOLD}monitor-check${NC} <id>    Manual check specific match"
    echo ""
    
    echo -e "${CYAN}━━━ DEVELOPMENT COMMANDS ━━━${NC}"
    echo "  ${BOLD}test${NC}                  Run all tests"
    echo "  ${BOLD}test-v${NC}                Run tests with verbose output"
    echo "  ${BOLD}sync${NC}                  Sync dependencies (uv sync)"
    echo "  ${BOLD}shell${NC}                 Open uv shell"
    echo ""
    
    echo -e "${MAGENTA}━━━ UTILITY COMMANDS ━━━${NC}"
    echo "  ${BOLD}status${NC}                Show project status"
    echo "  ${BOLD}env-check${NC}             Check environment setup"
